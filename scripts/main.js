...
function saveBookmark(hikeDocID) {
  currentUser.get().then((userDoc) => {
    ...
    if (bookmarks.includes(hikeDocID)) {
      // Manage the backend process to remove the hikeDocID from the database, recording which hike was bookmarked by the user.
      currentUser
        .update({
          // Use 'arrayRemove' to remove the bookmark ID from the 'bookmarks' array.
          // This method ensures that the ID is removed only if it's already present, preventing errors.
